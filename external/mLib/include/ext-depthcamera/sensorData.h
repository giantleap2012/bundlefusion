...
#define M_SENSOR_DATA_VERSION 4
		...
		void initDefault(
			...
			COMPRESSION_TYPE_COLOR colorType = TYPE_PNG,
			COMPRESSION_TYPE_DEPTH depthType = TYPE_ZLIB_USHORT,
		...
		//! 7-scenes format
		void loadFromImages(const std::string& sourceFolder, const std::string& basename = "", const std::string& colorEnding = "png")
		{
			...
			StringCounter scDepth(sourceFolder + "\\" + basename, "depth.pgm", 6);
			...
			for (unsigned int i = 0;; i++) {
				...
				if (!ml::util::fileExists(colorFile) || !ml::util::fileExists(depthFile)){
					//|| !ml::util::fileExists(poseFile)) {
				...
				ml::SensorData::COMPRESSION_TYPE_DEPTH compressionDepth = ml::SensorData::COMPRESSION_TYPE_DEPTH::TYPE_ZLIB_USHORT;

				initDefault(m_colorWidth, m_colorHeight, m_depthWidth, m_depthHeight, m_calibrationColor, m_calibrationDepth);
